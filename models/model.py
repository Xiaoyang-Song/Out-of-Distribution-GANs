...
class PCDiscriminator(nn.Module):
    def __init__(self, num_classes=4):
        ...
        self.block1 = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=(5, 3), padding=(2, 0)),
            nn.BatchNorm2d(32),
            nn.ReLU(),
    ...
    def forward(self, x):
        # Input: [batch_size, 1, 300, 3]
        x = self.block1(x)   # -> [batch, 32, 300, 1]
        x = self.block2(x)   # -> [batch, 64, 300, 1]
        x = self.block3(x)   # -> [batch, 128, 300, 1]
