...
def mnist_fashion_mnist():
    ...
    mnist_img = torch.stack([x[0] / torch.sum(x[0])
                             for x in mnist]).flatten(1, 3)[0:10000]
    mnist_label = torch.ones(mnist_img.shape[0])

    fashionmnist, _, _, _ = FashionMNIST(128, 64, sf=True)
    fashionmnist_img = torch.stack([x[0] / torch.sum(x[0])
                                    for x in fashionmnist]).flatten(1, 3)[0:10000]
    fashionmnist_label = torch.zeros(fashionmnist_img.shape[0])
    x_vis = torch.cat([mnist_img, fashionmnist_img])
    y_vis = torch.cat([mnist_label, fashionmnist_label])
    ic(x_vis.shape)
    ic(y_vis.shape)
    umap_visualization(x_vis, y_vis, ['#00b384', 'red'])
